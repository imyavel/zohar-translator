...
> Этот файл — полная инструкция по CLI для словаря проекта Зоар.
...
Если **любая** операция падает (правила нет, конфликт при `add`, и т. п.) — **ни одна** правка не сохраняется. Успешный прогон — одна новая версия `glossary_{N+1}.json`.
...
- Каждая мутация (`add`/`update`/`delete`/`batch-*`/`set-meta`) создаёт
  `glossary_{NNN+1}.json`.
- Старые версии **не удалять** — это история; `diff` ими пользуется.
- Текущая версия обнаруживается автоматически (`find_latest_glossary`).
